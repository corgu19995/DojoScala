...
object WebServer{
  ...
  implicit val noteFormat = jsonFormat(Note)
  object MyJsonStudentProtocol extends DefaultJsonProtocol{
    implicit def studentFormat[Note: JsonFormat] = jsonFormat3(Student)
  }
  object MyJsonCourseProtocol extends DefaultJsonProtocol{
    implicit def courseFormat[Student: JsonFormat] = jsonFormat3(Student)
  }
  ...
  object Operation{
    ...
    def fetchStudents(courseId: String):Future[Option[List[Student]]] = Future{
      courses.find(c => c.id == courseId.flatMap(course => Option(students)))
    }
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    val bindFuture = Http().bindAndHandle("localhost",8080.toString)
